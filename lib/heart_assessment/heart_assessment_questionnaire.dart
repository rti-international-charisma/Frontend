...
class _HeartAssessmentQuestionaireState extends State<HeartAssessmentQuestionnaireWidget> {
                                ...
                                child: ElevatedButton(
                                    ...
                                    onPressed: () {
                                      if (currentDisplaySection <
                                        ...
                                        showTestDonePopup(heartAssessment, widget.apiClient);
                                      }
                                    },
  ...
  HeartAssessmentResult createResultObject(HeartAssessment heartAssessment) {
    List<Section> sectionsList = result.map((sectionId, QnAMap) {
      ...
      var section = Section(sectionId: sectionId, sectionType: getSectionTypeFromSectionId(heartAssessment, sectionId)!, questions: answers);
  ...
  Future<void> showTestDonePopup(HeartAssessment heartAssessment, ApiClient apiClient) async{
                    ...
                    child: ElevatedButton(
                        onPressed: () async {
                          ...
                          if(token != null) {
                            ...
                            await apiClient.postWithHeaders('/assessment/scores', results,
                                {
                              ...
                              ScaffoldMessenger.of(_scaffoldKey.currentContext!).showSnackBar(
                                  SnackBar(
                                    content: Text('There was an error while submitting the result. Please try again'),
                                    backgroundColor: Colors.red,
                                  )),
                              Navigator.pop(context)
                            });
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: results.sections.map((section)  =>
                  Text('${section.sectionType} ${section.questions.fold(0, (previousValue, element) => (previousValue as int) + element.score)}')
              ).toList(),
            ),
          );
        }
